    ...
    <footer className="bg-gray-900 text-gray-300">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div className="lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              />
              <span className="text-xl font-bold text-white">Axolotly</span>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              Protection that grows with your family. Giving parents the tools to create safe, meaningful digital experiences for their children.
            </p>
            <div className="flex gap-4">
              <a href="#" className="text-gray-500 hover:text-[#F77B8A] transition-colors" aria-label="Twitter">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
              <a href="#" className="text-gray-500 hover:text-[#F77B8A] transition-colors" aria-label="Facebook">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href="#" className="text-gray-500 hover:text-[#F77B8A] transition-colors" aria-label="Instagram">
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li><button onClick={() => router.push('/how-it-works')} className="text-gray-400 hover:text-white transition-colors text-sm">How It Works</button></li>
              <li><button onClick={() => router.push('/pricing')} className="text-gray-400 hover:text-white transition-colors text-sm">Pricing</button></li>
              <li><button onClick={() => router.push('/parent')} className="text-gray-400 hover:text-white transition-colors text-sm">Parent Dashboard</button></li>
              <li><button onClick={() => router.push('/kids')} className="text-gray-400 hover:text-white transition-colors text-sm">Kids Launcher</button></li>
              <li><button onClick={() => router.push('/about')} className="text-gray-400 hover:text-white transition-colors text-sm">About</button></li>
            </ul>
          </div>
        ...
        <div className="border-t border-gray-800 mt-12 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-gray-500 text-sm">&copy; {new Date().getFullYear()} Axolotly. All rights reserved.</p>
          <p className="text-gray-500 text-sm">Built with love and protection.</p>
        </div>
      </div>
    </footer>
  );
